...
# check if a sourcemap url has been supplied
if [ -z "$1" ]; then
...
# store path without sourcemap filename
URL=$(echo $1 | rev | cut -d '/' -f2- | rev)
echo $URL;
...
echo "Map loaded: read $(echo $MAP | wc -c) bytes from $(echo $1 | rev | awk -F'/' '{print $1}' | rev)."
